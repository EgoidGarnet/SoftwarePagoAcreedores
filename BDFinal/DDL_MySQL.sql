...
CREATE TABLE IF NOT EXISTS `softpacbd`.`PA_USUARIOS` (
  ...
  `USUARIO_ELIMINACION` INT UNSIGNED NULL,
  PRIMARY KEY (`USUARIO_ID`),
  INDEX `fk_PA_USUARIOS_PA_USUARIOS1_idx` (`USUARIO_ELIMINACION` ASC) VISIBLE,
  UNIQUE INDEX `NOMBRE_DE_USUARIO_UNIQUE` (`NOMBRE_DE_USUARIO` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `softpacbd`.`PA_USUARIO_PAIS_ACCESO` (
  ...
  INDEX `fk_PA_USUARIO_PAIS_ACCESO_PA_PAISES1_idx` (`PAIS_ID` ASC) INVISIBLE,
  INDEX `fk_PA_USUARIO_PAIS_ACCESO_PA_USUARIOS1_idx` (`USUARIO_ID` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `softpacbd`.`PA_ESTADO_CUENTAS_PROPIAS` (
  ...
  `USUARIO_MODIFICACION` INT UNSIGNED NOT NULL,
  INDEX `fk_PA_KARDEX_CUENTAS_PROPIAS_PA_USUARIOS1_idx` (`USUARIO_MODIFICACION` ASC) VISIBLE,
  INDEX `fk_PA_KARDEX_CUENTAS_PROPIAS_PA_CUENTAS_PROPIAS1_idx` (`CUENTA_PROPIA_ID` ASC) VISIBLE,
...
DELIMITER $$
USE `softpacbd`$$
CREATE PROCEDURE pa_sp_reporte_facturas_pendientes(
